...
# except:
#     from csm_triton import cross_scan_fn, cross_merge_fn
from model.csm_triton import cross_scan_fn, cross_merge_fn
...
class GSC(nn.Module):
    ...
    def forward(self, x):
        ...
        out_ = x + x_residual
        out = out_.permute(0, 2, 3, 4, 1).contiguous()
        return out
...
#         # # 对 h 维度上的所有切片进行操作
#
#         xh = self.sh(input.reshape(-1, h, w*d, c)).reshape(b, h, w, d, c)
#         xw = self.sw(input.permute(0, 2, 1, 3, 4).reshape(-1, w, h*d, c)).reshape(b, w, h, d, c).permute(0, 2, 1, 3, 4)
#         xd = self.sd(input.permute(0, 3, 1, 2, 4).reshape(-1, d, h*w, c)).reshape(b, d, h, w, c).permute(0, 2, 3, 1, 4)
#
#         output = xh + xw + xd
#         return output
...
class VSSM3D(nn.Module):
    def __init__(
        ...
        super().__init__()
        self.channel_first = (norm_layer.lower() in ["bn", "ln3d"])
        self.num_classes = num_classes
        self.num_layers = len(depths)
        if isinstance(dims, int):
            dims = [int(dims * 2 ** i_layer) for i_layer in range(self.num_layers)]
    ...
    def _make_patch_embed_v3(in_chans=3, embed_dim=96, patch_size=4, patch_norm=True, norm_layer=nn.LayerNorm,
        ...
        padding = 1
        return nn.Sequential(
            nn.Conv3d(in_chans, embed_dim // 2, kernel_size=kernel_size, stride=stride, padding=padding),
            ...
            nn.GELU(),
            nn.Conv3d(embed_dim // 2, embed_dim, kernel_size=kernel_size, stride=stride, padding=padding),
            (nn.Identity() if channel_first else Permute(0, 2, 3, 4, 1)),
            (norm_layer(embed_dim) if patch_norm else nn.Identity()),
        )

    @staticmethod
    def _make_patch_embed_v4(in_chans=3, embed_dim=96, patch_size=4, patch_norm=True, norm_layer=nn.LayerNorm,
                             channel_first=False):
        # if channel first, then Norm and Output are both channel_first
        stride = patch_size // 2
        kernel_size = stride + 1
        ...
        return nn.Sequential(
            ...
            (nn.Identity() if (channel_first or (not patch_norm)) else Permute(0, 4, 1, 2, 3)),
            nn.GELU(),
            nn.Conv3d(embed_dim // 2, embed_dim, kernel_size=1, stride=1, padding=0),
            (nn.Identity() if channel_first else Permute(0, 2, 3, 4, 1)),
            (norm_layer(embed_dim) if patch_norm else nn.Identity()),
        )
    ...
    @staticmethod
    def _make_downsample_v4(dim=96, out_dim=192, norm_layer=nn.LayerNorm, channel_first=False):
        # if channel first, then Norm and Output are both channel_first
        return nn.Sequential(
            (nn.Identity() if channel_first else Permute(0, 4, 1, 2, 3)),
            nn.Conv3d(dim, out_dim, kernel_size=3, stride=2, padding=1),
            (nn.Identity() if channel_first else Permute(0, 2, 3, 4, 1)),
            norm_layer(out_dim),
        )
    ...
    def _make_layer(
        ...
        blocks.append(GSC(dim))

        for d in range(depth):
            blocks.append(SABMamba(
                hidden_dim=dim,
                drop_path=drop_path[d],
                norm_layer=norm_layer,
                channel_first=channel_first,
